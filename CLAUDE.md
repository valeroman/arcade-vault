...
- `app/globals.css` — the entire design system, ~3400 lines of hand-written CSS (see **Design system** below).
...
- `app/games/{asteroids,tetris,arkanoid,snake,frogger}/page.tsx` — thin static wrappers around each game component
...
- **Cover art is CSS, not images:** `games.cover` stores a class name, rendered as `<div className={"cover-bg " + game.cover} />`. Orphaned reusable blocks from the deleted mock catalog: `.cover-glot`, `.cover-invaders`, `.cover-duelo` — reuse one before writing a new `.cover-*` (Arkanoid reused `.cover-bricks`, Frogger reused `.cover-rana`)
- **Game shell** (spec 10/11): `.game-arena`/`.game-screen`/`.game-canvas-wrap` wrap every canvas, `.game-bottom-bar` holds pause/skin-select/exit, `.game-crt`/`.game-crt-screen`/`.game-crt-bottom` render the CRT bezel — but `.game-crt`/`.game-crt-screen` only have rules under `body.av-touch-game`, so on desktop they're inert classes the wrappers render unconditionally. `.touch-*` (`.touch-controls`, `.touch-dpad`, `.touch-btn`, …) style `TouchControls.tsx`'s `TouchButton[]` layout.
...
- `components/games/<slug>/engine.ts` — framework-agnostic logic (lives under `components/`, not `lib/`). Contract: `createGame(canvas, { onGameOver, skin })` → `EngineHandle { restart, destroy, setSkin }` (the `skin`/`setSkin` half was added by `skin-designer`, see **Agents**). Tetris deviates: `createGame(canvas, nextCanvas, cb)` plus `onHud` (its original HUD was DOM-based); Snake, Arkanoid and Frogger draw their HUD on-canvas.
- `components/games/skins.ts` — shared skin registry: `SkinId` (`"clasico" | "neon" | "retro"`), `SkinGameId` (one per `games.id`), `SKINS: Record<SkinId, Skin>`, `DEFAULT_SKIN`, `getSkin()`/`readSkin()`/`writeSkin()` (persist to `localStorage`), `withAlpha()`. A `Skin.games[gameId]` override lets a game's palette diverge from the shared one when `clasico` must match that game's original, unrelated look.
- `components/games/TouchControls.tsx` — shared touch/mobile control surface (spec 10/11): renders a per-game `TouchButton[]` (`slot`, `mode: "hold" | "tap"`) as the d-pad + action buttons; each game wrapper defines its own layout and passes it in.
...
- Assets in `public/games/<slug>/` with absolute paths (`spritesheet-breakout.png`, `sounds/*.mp3`, `fruits.png`). Asteroids, Tetris and Frogger are 100% procedural — no assets.
...
- Current specs: `01`–`03` MVP/home/about (`Implementado`), `04` Supabase integration (`Aprobado`), `05` Asteroids (`Aprobado`), `06` leaderboard/games table (`Implementado`), `07` Tetris (`Implementado`), `08` Arkanoid (`Aprobado`), `09` Snake (`Implementado`), `10` mobile touch controls (`Implementado`), `11` gamepad visual redesign (`Implementado`).
- The four games under `references/resources/started-games/` (`02-asteroids`, `03-tetris`, `04-arkanoid`, `05-snake`) are already ported — the next `/add-game` run needs a new folder there. Frogger (`rana`) is the exception: it came from `game-jam`, not `/add-game`, so it has no `started-games/` source; its spec lives at `specs/game-jam/frogger/01-frogger-core.md` in blockquote-metadata format (not the `specs/NN-slug.md` YAML-frontmatter shape) and was never promoted into the numbered sequence.
...
- `/spec-impl-game <NN-slug>` — local skill (not tracked in `skills-lock.json`), lives in `.claude/skills/spec-impl-game/`. Delegates the implementation by reading `.claude/skills/spec-impl/SKILL.md` and running its 4 phases as-is (doesn't duplicate it — that skill is locked and `disable-model-invocation`), then verifies the acceptance criteria and chains `skin-designer <juego>`, then `mobile-porter <juego>`, and finally `game-performance <juego>` — sequential, never parallel. Stops the chain if the state isn't `Aprobado`, a criterion fails, or the spec isn't a game's. A `❌` gate from `game-performance` does **not** block the chain from closing — it's registered for a human to act on later. Real cadence for a full game port: `game-planner` → `/add-game <carpeta>` → human approval → `/spec-impl-game NN-slug` (includes `skin-designer` + `mobile-porter` + `game-performance`) → one branch = one PR.
- `.agents/skills/` is a byte-for-byte mirror of `.claude/skills/`, and `.agents/agents/` mirrors `.claude/agents/` the same way — keep both in sync when touching a skill or agent. The mirror does **not** extend to `.claude/hooks/` or `.claude/settings*.json`, which have no `.agents/` counterpart.
- All four project skills (`add-game`, `spec`, `spec-impl`, `spec-impl-game`) declare `disable-model-invocation: true` — none of them trigger automatically; they only run via their explicit `/command`.

## Agents

All defined in `.claude/agents/<name>.md`, mirrored byte-for-byte in `.agents/agents/`. Full rules, prohibitions, and quality gates live in each `.md` — this table is just the index.

| Agent              | What it does                                                                                                                                                                                                                                                                        | Memory (only file it writes besides game code)                                                       |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `game-planner`     | Decides which game to add next: reads the catalog/`specs/`/`started-games/`, detects gaps (category, color, difficulty, mechanic), proposes candidates. Never writes code, SQL, or specs.                                                                                           | [`references/resources/game-suggestions-todo.md`](references/resources/game-suggestions-todo.md)     |
| `game-jam`         | Given a theme, invents an original fully-procedural game and writes 2–3 alternative `Draft` specs (differing mechanic/scoring/HUD) into `specs/game-jam/<game-id>/`. Never writes code, applies SQL, or marks a spec `Aprobado`.                                                    | —                                                                                                    |
| `skin-designer`    | Applies the 3 mandatory skins (`clasico`/`neon`/`retro`) to one named game: extracts hardcoded colors into `components/games/skins.ts`, threads `skin`/`setSkin()` through the engine, adds a persisted selector.                                                                   | [`references/resources/game-with-themes.md`](references/resources/game-with-themes.md)               |
| `mobile-porter`    | Ports one named game to touch/mobile (spec 10/11 pattern): `TouchControls`, CRT bezel, bottom bar, responsive canvas scaling. Only allowed `engine.ts` edit: normalizing `e.key`/`e.keyCode` to `e.code`.                                                                           | [`references/resources/game-with-mobile.md`](references/resources/game-with-mobile.md)               |
| `game-performance` | Audits and fixes render performance for one named game via static op-counting (no profiler) against 10 quality gates (loop, allocs, draw calls, context leaks, sprites, re-renders, collisions, audio, input, teardown).                                                            | [`references/resources/game-perf-audit.md`](references/resources/game-perf-audit.md)                 |
| `security-auditor` | Audits security for one named area (`auth`/`datos`/`api`/`config`/`cliente`) against 17 quality gates with cited `file:line` evidence, classifies findings by severity, proposes remediation. Never writes application code, applies SQL, or marks a spec `Aprobado` — report-only. | [`references/resources/security/security-audit.md`](references/resources/security/security-audit.md) |

Real cadence for a full game port: `game-planner` → `/add-game <carpeta>` → human approval → `/spec-impl-game NN-slug` (chains `skin-designer` → `mobile-porter` → `game-performance`, sequential, never parallel; stops early if the state isn't `Aprobado`, a criterion fails, or the spec isn't a game's — a `❌` gate from `game-performance` does **not** block the chain, it's registered for a human to act on later) → one branch = one PR. None of the five ever processes more than one game per invocation. `skin-designer`, `mobile-porter`, and `game-performance` are the only ones that write code; only `game-performance` may touch mechanics, and only in two narrow framerate-dependence cases (G2/G10), never balance/scoring/difficulty, and only after an explicit confirmation separate from its general one. `security-auditor` is not part of this game-port cadence — it audits one application area per invocation (not a game) and never writes code at all, unlike the other three code-writing agents.
...
- `components/games/AsteroidsGame.tsx` (963 lines) has its engine **inline** — it was the first port. The four later games separate `engine.ts`; follow that pattern, not Asteroids'.
...
- `.claude/skills/add-game/porting-guide.md` is stale — it still says Asteroids (684 lines) is the only ported game; 5 are shipped today.
...
- Frogger is the only game without a mobile port: it never ran through `mobile-porter`, doesn't use `TouchControls`/`.game-crt`/`.game-bottom-bar`, and is absent from `references/resources/game-with-mobile.md`.
- `references/resources/game-suggestions-todo.md` (the `game-planner` memory) is stale on Frogger: it's still listed under "Pendientes" (with the wrong color, `yellow` instead of the shipped `green`) and missing from the "Implementados" table.
- No spec documents the Frogger skins work (commit `97f5b97`) — only its core spec (`specs/game-jam/frogger/01-frogger-core.md`) exists.
- `.game-crt`/`.game-crt-screen` (in `app/globals.css`) have no base rule — they only render under `body.av-touch-game`, yet all game wrappers apply those classes unconditionally.
