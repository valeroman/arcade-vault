...
- `mobile-porter` (`.claude/agents/mobile-porter.md`, mirrored in `.agents/agents/`) — the other agent that writes code. Given the name of a single game, ports it to touch/mobile by replicating spec 10's pattern: adds `TouchControls` with a per-game `TouchButton[]` layout, the `.game-crt` CRT bezel, the `.game-bottom-bar` (pause/skin-select/exit) and the responsive canvas scaling. Its memory is [`references/resources/game-with-mobile.md`](references/resources/game-with-mobile.md), the only file it writes besides the game's own code. Runs **after** `/spec-impl` and `skin-designer` in the chain. Never touches mechanics/balance/scoring, SQL, or binary assets, and never processes more than one game per invocation; the only allowed edit to an `engine.ts` is normalizing `e.key`/`e.keyCode` reads to `e.code`.
- `game-performance` (`.claude/agents/game-performance.md`, mirrored in `.agents/agents/`) — audits and fixes render performance for a single game via static, code-read op-counting (no profiler, no new deps): the game loop, per-frame allocations, draw-call volume, canvas context-state leaks, sprite/scaling costs, React re-renders, collision correctness under the clamped `dt`, audio, input (`preventDefault`/focus guards) and teardown/async-load safety — 10 quality gates in total. Its memory is [`references/resources/game-perf-audit.md`](references/resources/game-perf-audit.md), the only file it writes besides the game's own code (plus additive-only changes to shared files like `components/games/skins.ts`, never changing a signature or a value another game consumes). Runs standalone on demand, and **after** `mobile-porter` when chained from `/spec-impl-game` (mobile-porter's canvas scaling and touch compositing are the last thing to change render cost). Unlike the other two code-writing agents, it **is** allowed to touch mechanics in two narrow, framerate-dependence cases (an unscaled-by-`dt` drag/friction factor, or a collision resolution that skips positional correction) — never balance, scoring, difficulty, or design speeds — and only after an explicit confirmation separate from its general one.
