...
- **Tabla `games` en Supabase** (`supabase/schema.sql`) — columnas migradas de `app/data/games.ts` (`id`, `title`, `short`, `long`, `cat`, `cover`, `color`, `best`, `plays`), poblada con los 8 juegos actuales como datos reales (no seed ficticio).
...
- **`app/data/games.ts`** — pasa de exportar el array estático `GAMES` a funciones `getGames()` / `getGame(id)` que consultan Supabase. `CATS` se deriva de los juegos obtenidos en vez de estar hardcodeado.
...
- **`components/games/AsteroidsGame.tsx`** — al perder la última vida, overlay HTML sobre el canvas con input de nombre + botón "GUARDAR" que inserta el score directo desde el cliente browser de Supabase.
...
- **Fusión `rocas` = Asteroids** — la fila `rocas` de la tabla `games` representa el juego real. El botón "JUGAR AHORA" en `/games/rocas` apunta a `/games/asteroids`. Se elimina la tarjeta hardcodeada duplicada de Asteroids en `/games` (ya la cubre la tarjeta de `rocas` generada desde `GAMES`).
- **`app/games/[id]/play/page.tsx`** — se quita la llamada a `saveScore()` y el paso "GUARDAR PUNTUACIÓN" de esa pantalla (queda como demo visual sin persistencia); sigue accesible para los 7 juegos mock, pero ya no escribe en la tabla `scores`.
...
- Envío de puntajes desde los demás 7 juegos del catálogo (no tienen gameplay real todavía).
- Cálculo en vivo de "Mejor global" / "Partidas" desde `scores` — siguen siendo columnas fijas de `games`.
...
- Reemplazar o rediseñar la pantalla simulada `/games/[id]/play` — solo se le quita el guardado, sin más cambios.
- Tests.
...
| Juegos que envían score real       | Solo Asteroids (`rocas`)                                    | Los 8 juegos                                  | Es el único con gameplay real; los demás son mock                     |
...
| UX de nombre al perder             | Overlay HTML con input sobre el canvas                      | Reusar nombre de localStorage automáticamente | Explícito y simple; no depende de `app/data/user.ts`                  |
| Stats "Mejor global" / "Partidas"  | Columnas fijas en `games` (igual que hoy)                   | Calculadas en vivo desde `scores`             | Evita agregaciones; el leaderboard real vive en `scores` aparte       |
| Datos iniciales                    | `games` con datos reales, `scores` vacía                    | Seed de puntajes de ejemplo                   | Estado honesto: sin partidas reales todavía no hay puntajes           |
...
| `/games/[id]/play` (demo simulada) | Se le quita el guardado, sigue como demo visual             | Eliminar la ruta completa                     | Cambio mínimo; no afecta a los 7 juegos mock                          |
