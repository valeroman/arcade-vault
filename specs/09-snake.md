...
4. **`.btn.green`** — añadir la variante a `app/globals.css` junto a `.btn.magenta`/`.btn.yellow`: `border-color: var(--green)` y hover con `box-shadow`/`color` en verde, mismo patrón exacto que las dos variantes existentes.
