...
- Fila de Arkanoid en la tabla `games` de Supabase (id `ladrillos`, `route = '/games/arkanoid'`;
  la columna `route` ya existe desde la spec 07, no requiere migración. `id` ≠ slug de
  `route` a propósito — ver Decisiones).
...
- Guardado real de puntaje: `submitScore("ladrillos", nombre, score)` al perder o
  al ganar (completar los 5 niveles), visible en `/games/ladrillos` (detalle/leaderboard
  vía `[id]`) y `/hall-of-fame`.
...
1. **Fila en Supabase** — añadir el `insert` (bloque de arriba) a `supabase/schema.sql` y aplicarlo con el MCP `supabase`.
   _Verificación:_ `select * from games where id = 'ladrillos'` devuelve la fila.
...
5. **`components/games/ArkanoidGame.tsx`** — wrapper siguiendo el patrón de `TetrisGame.tsx`/`AsteroidsGame.tsx`: estados `finalScore/name/saving/saved/saveError`, `useEffect` que monta `createGame(canvasRef.current, { onGameOver })` y desmonta con `destroy()`, guard de teclado para inputs, modal reutilizando `.modal-bd`/`.modal`/`.final`/`.input-row`/`.actions`/`.toast-saved`, `submitScore("ladrillos", …)`, enlace "VER RANKING" a `/games/ladrillos` (detalle/leaderboard vía `[id]`), botón "JUGAR DE NUEVO" llamando `restartRef.current?.()`.
...
7. **Prueba de extremo a extremo** — jugar una partida completa (perder con las 3 vidas y también completar los 5 niveles en otra corrida), guardar el puntaje, verlo reflejado en `/hall-of-fame` y en `/games/ladrillos`.
...
- [ ] Guardar el puntaje inserta una fila real en `scores` con `game_id = 'ladrillos'`.
- [ ] Escribir un espacio en el input de nombre no reinicia el juego.
- [ ] El puntaje guardado aparece en `/hall-of-fame` (tab correspondiente) y en `/games/ladrillos`.
- [ ] `/games/ladrillos` (vía `/games/[id]`) muestra "JUGAR AHORA" apuntando a `/games/arkanoid`, sin ser tapada por la ruta estática jugable.
...
| ------------------------------------ | ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id` de Supabase vs. slug de `route` | `id='ladrillos'`, `route='/games/arkanoid'` | Detectado durante el Paso 4 de `/spec-impl`: la spec original fijaba `id='arkanoid'` con `route='/games/arkanoid'` — misma URL. La ruta estática (`app/games/arkanoid/page.tsx`) siempre gana sobre la dinámica `[id]` para la misma URL, así que la página de detalle/leaderboard hubiera quedado inalcanzable. Se corrige separando `id` del slug, igual que rocas→asteroids y tetro→tetris. |
